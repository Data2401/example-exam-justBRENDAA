...
Saving - Storing changes made to files in our local device. 

Committing - is like creating checkpoints of our work. A snapshot of our work at a given time. These changes are only local. 

Pushing - Uploading our commits to github so they are no longer local changes only. 
...
# numbers / 2
# dividing the new numbers vector by two will not work because it contains strings 
# vectors must have same data type so by adding "puppies" all other elements were converted to strings 
...
```{r}
# finding average method 1
average_age <- sum(age)/length(age)
average_age

# finding average method 2
average_age2 <- mean(age)
average_age2
...
# oldest dog
old_dog <- names[which.max(age)]
old_dog

# heaviest dog
big_dog <- names[which.max(weight)]
big_dog
...
```{r}
slugging_percentage <- function(singles, doubles, tripples, hr, ab){
  numerator <- (singles + (doubles * 2) + (tripples * 3) + (hr * 4))
  denominator <- ab
...
sqrt_v2 <- function(x){
  root <- x^(1/2)
  root <- replace(root, is.nan(root), 0)
  round(root, 2)
}
...
```{r}
bonus_v2 <- function(score, salary){
  if (score == 1){
    return(1.05*salary)
  } else if(score == 2){
    return (1.10 * salary)
